    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
        <div style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,0.08)', marginBottom: 20 }}>
          {['edit', 'preview'].map(t => (
            <div key={t} onClick={() => setTab(t)} style={{ padding: '8px 18px', fontSize: 13, cursor: 'pointer', color: tab === t ? 'var(--text)' : 'var(--text2)', fontWeight: tab === t ? 500 : 400, borderBottom: tab === t ? '2px solid var(--accent)' : '2px solid transparent', marginBottom: -1, textTransform: 'capitalize' }}>
              {t}
            </div>
          ))}
        </div>
        ...
        {tab === 'edit' && (
          <>
            ...
            <div className="field">
              ...
              <textarea value={template.body} onChange={e => set('body', e.target.value)} style={{ minHeight: 280 }} />
            </div>
            <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: -6 }}>
              Use <code style={{ background: 'var(--surface2)', padding: '1px 5px', borderRadius: 4 }}>{'{name}'}</code> and <code style={{ background: 'var(--surface2)', padding: '1px 5px', borderRadius: 4 }}>{'{company}'}</code> for personalization.
            </div>
          </>
        )}
        ...
        {tab === 'preview' && (
          <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, overflow: 'hidden' }}>
            <div style={{ background: 'var(--surface2)', padding: '12px 18px', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ padding: 18, fontFamily: 'var(--mono)', fontSize: 12, lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
